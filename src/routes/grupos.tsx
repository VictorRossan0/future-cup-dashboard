  ...
  head: () => ({
    meta: [
      { title: "Grupos e Classificação · Copa 2026 Intelligence" },
      {
        name: "description",
        content:
          "Classificação dos 12 grupos da Copa 2026 com pontos, vitórias, saldo de gols e status de classificação.",
      },
    ],
  }),
    ...
    <AppLayout>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10 space-y-8">
        <PageHeader
          ...
          description={
            <>
              12 grupos com 4 seleções cada. Os <span className="text-primary font-medium">dois primeiros</span> avançam
              direto e os <span className="text-gold font-medium">8 melhores terceiros</span> também se classificam.
            </>
          }
        ...
        {q.isLoading ? (
          ...
          <EmptyState
            ...
            description="Popule v_groups_standings para visualizar a classificação."
          />
        ) : (
          <div className="grid md:grid-cols-2 gap-5">
            {grouped.map(([letter, rs]) => (
              <GroupTableView key={letter} letter={letter} rows={rs} />
            ))}
          </div>
        )}
      </div>
